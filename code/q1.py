...
def generate_mosaic_AFI_pp(AFI: np.ndarray) -> None:
    An = AFI.shape[0]
    Fn = AFI.shape[-1]
    ...
    for i in range(An):
        for j in range(Fn):
            plt.subplot(An, Fn, count)
            plt.imshow(AFI[i,y,x,:,j].reshape((1,1,3)))
            plt.axis('off')
            count +=1
    ...
    plt.subplots_adjust(left=0,
                    ...
                    wspace=0,
                    hspace=0)
    plt.savefig("./../../output/1_5_pp.png", bbox_inches='tight', dpi=2000)
    #plt.show()
...
def confocal_sterio(light_field, depths, apertures):
    ...
    #generate_mosaic_AFI(AFI)
    generate_mosaic_AFI_pp(AFI)
...
if __name__=="__main__":
    ...
    for i in range(integrated_image.shape[3]):
        cv2.imwrite(f'./../../output/1_3_intergrated_{i}.png', integrated_image[:,:,:,i])
